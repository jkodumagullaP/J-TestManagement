...
using CTWebsite.WebDriver.SwdBrowserUtils;
using CTWebsite.FunctionalTesting;

namespace CTWebsite.WebDriver
{
    public static class SwdBrowser
    {
        ...
        {
            AddNewTestCase atc = new AddNewTestCase();
            //_driver = atc.driver1;
            if (_driver != null)
            {
                _driver.Quit();
        ...
        public static void InjectVisualSearch()
        {
            lock (lockObject)
            {
                JavaScriptUtils.InjectVisualSearch(_driver);
            }
        }
        ...
        public static void DestroyVisualSearch()
        {
            lock (lockObject)
            {
                JavaScriptUtils.DestroyVisualSearch(_driver);
            }
        }
        ...
        public static bool IsVisualSearchScriptInjected()
        {
            lock (lockObject)
            {
                return JavaScriptUtils.IsVisualSearchScriptInjected(_driver);
            }
        }
        ...
        public static void HighlightElement(By by)
        {
            lock (lockObject)
            {
                JavaScriptUtils.HighlightElement(by, _driver);
            }
        }
        ...
        public static HtmlDocument GetPageSource()
        {
            lock (lockObject)
            {
                return HtmlPageUtils.GetPageSource(_driver);
            }
        }
        ...
        public static BrowserCommand GetNextCommand() // Returns null if no new commands received
        {
            ...
            lock (lockObject)
            {
                return BrowserCommands.GetNextCommand(_driver);
            }
        }
