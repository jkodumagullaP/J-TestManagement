...
namespace CTWebsite.WebDriver.SwdBrowserUtils
{
    public static class WebDriverUtils
    {
        ...
        public static IWebDriver Initialize(WebDriverOptions browserOptions, out bool isRemote)
        {
            ...
            browserOptions.IsRemote = true;
            if (browserOptions.IsRemote)
            {
                driver = ConnetctToIWebDriver(browserOptions);
                isRemote = true;
            }
            else
            {
        ...
        private static IWebDriver ConnetctToIWebDriver(WebDriverOptions browserOptions)
        {
            ...
            caps = ConfigureIWebDriverCapabilities(caps, browserOptions.BrowserProfile, isRemoteDriver);

            IWebDriver newDriver = new RemoteWebDriver(hubUri, caps);
            return newDriver;
        }
        ...
        private static DesiredCapabilities ConfigureIWebDriverCapabilities(DesiredCapabilities caps, Profile browserProfile, bool isRemoteDriver)
        {
            ...
            if (isRemoteDriver)
            {
                foreach (string[] capability in profileCaps.IWebDriver)
                {
